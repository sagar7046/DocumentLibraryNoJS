...
const BaseRequest = (
  ...
  return BaseClient(url, { headers, method, body }).then((r) => r);
};
...
const GetJson = (
  ...
  return BaseClient(url, { headers, method, body }).then((r) => r.json());
};
 ...
 */
export const SPFileUpload = async ({
  url,
  payload = {},
  hdrs = {},
  digest = defaultDigest,
...
export const SPMultiFileUpload = ({
  url,
  payload = {},
  hdrs = {},
  files = [],
  ...
  return new Promise(async (resolve, reject) => {
    ...
    for (var j = 0; j < files.length; j++) {
      ...
      promiseAll.push(abc);
      if (promiseAll.length == files.length) resolve(promiseAll);
    }
  });
};
...
let GetUniqueFileData = () => {
  return new Blob([GetUniqueFileName()]);
};
